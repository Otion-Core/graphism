defmodule Graphism.Rest do
  ...
  def handler_modules(schema, repo_module, caller_module) do
    ...
    Enum.flat_map(schema, fn e ->
      standard_actions_handler_modules(e, opts) ++
        aggregation_handler_modules(e) ++
        list_children_handler_modules(e, opts) ++
        list_by_non_unique_key_handler_modules(e) ++
        list_by_custom_queries_handler_modules(e, opts) ++
        read_by_unique_key_modules(e) ++
  ...
  defp standard_actions_handler_modules(e, opts) do
    [
      list_handler_module(e),
      read_handler_module(e),
      create_handler_module(e, opts),
      update_handler_module(e, opts),
      delete_handler_module(e)
    ]
  end

  defp aggregation_handler_modules(e) do
    [
      aggregate_all_handler_module(e)
    ]
  end

  defp read_by_unique_key_modules(e) do
    e
    |> Entity.unique_keys_and_attributes()
    |> Enum.map(&read_by_unique_key_module(e, &1))
  end
  ...
  defp list_by_non_unique_key_handler_modules(e) do
    e
    |> Entity.non_unique_keys()
    |> Enum.flat_map(fn key ->
      [
        list_by_non_unique_key_handler_module(e, key),
        aggregate_by_non_unique_key_handler_module(e, key)
      ]
    end)
  end
  ...
  defp list_handler_module(e) do
    body =
      quote do
        def handle(conn, _opts) do
          with :ok <- allowed?(unquote(e[:name]), :list, conn.assigns),
    ...
    handler_module(e, :list, body)
  end

  defp aggregate_all_handler_module(e) do
    body =
      quote do
        def handle(conn, _opts) do
          with :ok <- allowed?(unquote(e[:name]), :read, conn.assigns),
    ...
    handler_module(e, :aggregate, body)
  end
  ...
  defp list_children_handler_module(e, rel, opts) do
    ...
    body =
      quote do
        def handle(conn, _opts) do
          with :ok <- allowed?(unquote(rel[:target]), :list, conn.assigns),
  ...
  defp list_by_non_unique_key_handler_module(e, key) do
  ...
  defp aggregate_by_non_unique_key_handler_module(e, key) do
  ...
  defp list_by_custom_query_handler_module(e, action, action_opts, opts) do
    ...
    body =
      quote do
        def handle(conn, _opts) do
          with :ok <- allowed?(unquote(e[:name]), unquote(action), conn.assigns),
  ...
  defp read_handler_module(e) do
    ...
    handler_module(e, :read, body)
  end

  defp read_by_unique_key_module(e, key) do
      ...
      quote do
        def handle(conn, _opts) do
          with unquote_splicing(args),
               :ok <- allowed?(unquote(e[:name]), :create, conn.assigns, args),
               {:ok, item} <-
  ...
  defp update_handler_module(e, opts) do
    ...
    handler_module(e, :update, body)
  end

  defp delete_handler_module(e) do
    body =
      quote do
        def handle(conn, _opts) do
  ...
  defp handler_module(e, action, body) do
